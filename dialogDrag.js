  ...
  bind(el, binding, vnode, oldVnode) {
    ...
    let moveDown = (e) => {
      ...
      document.onmouseup = function (e) {
        //如果header被移动到屏幕之外导致无法点击到header，则还原弹窗位置
        const rect = dialogHeaderEl.getBoundingClientRect();
    ...
    //     presentHeight = window.getComputedStyle(e.path[1]).height;
    ...
    //   } else {
    //     dragDom.style.height = presentHeight;
    ...
    dragDom.onmousemove = function (e) {
      ...
      dragDom.onmousedown = (e) => {
        ...
        let bodyH = parseInt(window.getComputedStyle(el.querySelector('.el-dialog__body')).height);
        ...
        if (clientX > EloffsetLeft && clientX < EloffsetLeft + elW && clientY > EloffsetTop && clientY < EloffsetTop + 100) {
          ...
          document.onmousemove = function (e) {
            ...
            if (ELscrollTop + clientY > EloffsetTop + elH - 20 && ELscrollTop + clientY < EloffsetTop + elH) {
              const bodyDom = dragDom.querySelector('.el-dialog__body');
              //往上拖拽
              if (clientY > e.clientY) {
                if (dragDom.clientHeight < minHeight) {
                } else {
                  bodyDom.style.height = bodyH - (clientY - e.clientY) + 'px';
                }
              }
              //往下拖拽
              if (clientY < e.clientY) {
                bodyDom.style.height = bodyH + (e.clientY - clientY) + 'px';
              }
            }
          };
          //拉伸结束
